...
ggplot(temp.data.sub_mean[temp.data.sub_mean$Channel == "Top2\u03b1" & temp.data.sub_mean$Mean > 100,], aes(y =Mean, x = Condition, fill = Channel)) +
  geom_text(aes(label=..count..), y=0, stat='count', colour="red", size=3)+
  geom_bar(stat="identity", width=0.5)+
  facet_wrap(~Concentration, scale="free")+
...
## Relative number of condensates per area unit
t = temp.data.sub_mean[temp.data.sub_mean$Channel == "Top2\u03b1" & temp.data.sub_mean$Mean > 500,]
